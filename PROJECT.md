...
- 🔴 <b>ЗАМЕР 01.09.2026: ЖИВОЙ ВИЗИТ НЕ ЗАПИСАЛСЯ.</b> Денис открыл сайт с телефона 13:28 (скрин
  ...
  Значит обрыв на КЛИЕНТСКОЙ стороне, между браузером и портом 8444. Две живые гипотезы, обе НЕ
  проверены: (1) порт 8444 режется сетью/оператором, (2) блокировщик или защита в браузере Дениса
  режет `script.js` с нестандартного порта. Разделяются одним действием — открыть
  `https://api.devexthub.com:8444/script.js` прямо в том же браузере: виден JS → порт жив, виноват
  блокировщик; не открывается → режется порт.
  <b>ВАЖНО ДЛЯ ВЫВОДОВ:</b> пока это не закрыто, «в Umami ноль» НЕ означает «трафика ноль».
  Прежний сквозной тест 26.08 шёл с самого сервера и клиентский путь не проверял.
