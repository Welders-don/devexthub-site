...
## СЕССИЯ 2026-08-07 — hero-анимации + иконки в карточках + бренд-знак (ждёт выбора)
### Что сделано и ЗАПУШЕНО в main (live на www.devexthub.com)
- **Hero-анимации, CSS-only** (коммит ba50e24): (1) пословный ревил H1 на входе — маска `.fx-line` + `.fx-w` translateY, стагген 70мс, одноразовый; (2) sheen по `.hero .btn-lg` — цикл 4.5с, только герой-кнопка. Обёрнуто в `@media (prefers-reduced-motion: no-preference)` → текст H1 виден и без анимаций (SEO/a11y ок). H1 размечены по словам на ВСЕХ 5: index, transcribe, pdf-to-excel, extract-text, image-enhancer. Прототип: `_preview/hero-fx.html`. Денис одобрил.
- **Иконки продуктов в карточках** главной (коммит 941036d): текстовые плашки `XL/VT/TX/IE` → реальные store-иконки из `assets/*-icon-128.png` (сверены с CWS, совпадают). CSS: `.tool img.mark {border:none;background:transparent;object-fit:cover}`. width/height + loading=lazy. Денис доволен — «живее, появилась индивидуальность».

### ОТКРЫТО — ждёт решения Дениса (он думает)
- **Бренд-знак Devexthub.** Цель: поднять узнаваемость САМОГО хаба (не только продуктов). Ключевой рычаг (объяснено Денису): единое семейство иконок + подпись «by Devexthub» на каждом листинге CWS — узнаваемость живёт в тулбаре/сторе, не на лендинге.
  - Набросано 3 направления знака (`_preview/logo-concepts.html` + .png): 1·хаб-узел, 2·монограмма «dx», 3·чеврон `<>`. 
  - Хаб-узел v1 был похож на зелёный X-крест PDF-иконки → сделал v2 с 5 формами (`_preview/logo-hub-v2.html` + .png): A·«+» 4 узла, B·«Y» 3 узла, C·гекс 6 (плохо в 18px), D·орбита без линий, E·«+» с кольцом-центром.
  - **Моя рекомендация: E** (силуэт «+», полый центр = «hub/port», не путается ни с чем). Запас: A (безопасный), B (максимально уникальный).
  - **СЛЕДУЮЩИЙ ШАГ когда Денис выберет букву:** собрать реальные ассеты (SVG + PNG 128/48/32/16), поставить фавикон на сайт + в угол к «Devexthub» вместо зелёной точки `.dot`. Рендер иконок делаю через chrome-headless-shell (полный chrome в песочнице падает — см. workspace/knowledge/headless-screenshot.md).
  - Devexthub-эмблемы пока НЕТ, генерю с нуля из выбранного концепта.

## СЕССИЯ 2026-08-05 (вечер-3) — YouTube ротация: ET шортс залит, дальше Image Enhancer
### Что сделано
- ET recipe шортс v1 ЗАЛИТ Денисом. Title «Copy text from any photo in one click #Shorts». Закреп-коммент со ссылкой на лендинг выдан (в комменте ссылка кликается сразу, в описании — нет, время-гейт канала).
- Метаданные ОБОИХ ET-шортсов (recipe v1 / paste v2) зафиксированы в releases/shorts/et-shorts-metadata.md (коммит 05174e4) — раньше жили только в чате и терялись. Там title/description/tags + закреп-коммент.
- Файлы шортсов: releases/video-samples/et-recipe-short-v1.mp4 (upload) и et-paste-short-v2.mp4 (Ctrl+V). Оба пересобраны 05.08 10:56 с эндкартой + звук -14 LUFS.
### Следующий шаг
- ЖДУ от Дениса OBS-экранку Image Enhancer (unblur/upscale реальных фото, размытое→чёткое) → монтирую длинный (широкоэкранка) + шортс.
- Календарь ротации: завтра (06.08) длинный IE, послезавтра (07.08) шортс IE. ET paste v2 (Ctrl+V) вклинить враздрай в свободный день.
- Ритм: широкоэкранки разброс 2 дня + шортсы разброс 2 дня, чередуя тему (ET→IE) — антиспам.
- Линк на залитый ET recipe шортс от Дениса → записать в growth-log, замерить охват.
...
## СЕССИЯ 2026-08-06 — стратегия каналов: ET-таран сейчас, IE приберечь на флагманский залп
### Решение Дениса (важно, не путать в будущем)
- Каналы привлечения ИСЧЕРПАНЫ (все директории/каталоги пройдены, лендинги отретушированы). Осталось два живых: Product Hunt (новый для нас) + YouTube-дрип.
- YouTube-цифры трезво: PDF-шортс 544 был ВЫБРОСОМ (разовый тест-разгон нового канала), не трендом — затухает нормально. ET шортс 8/12ч, ET длинный 1/сутки — ожидаемо для холодного канала. Прямых кликов шортс→лендинг ~0 (время-гейт описаний). YouTube сейчас качает только «взросление канала», не установки. НЕ убиваться, но и не жечь силы — дрип.
- IE НЕ лить сейчас никуда. Приберечь как ФЛАГМАНСКИЙ ЗАЛП: у Дениса на PH с каждым запуском растёт network (взаимные upvote, фолловеры) → запустить лучший продукт (IE, самый растущий 20+/день, зрелищный unblur before/after) по МАКСИМАЛЬНО разогретому network+каналу = максимальная отдача. «На разогретый канал лить то, что лучше всего перформит» (Денис).
- СЕЙЧАС таран = Extract Text (отстающий, терять нечего, заодно тянет network). ET по всем каналам синхронно: Product Hunt завтра + дрип ET paste-short v2 на YouTube. IE — единый залп PH+YouTube позже.
### Следующий шаг
- Готовлю Product Hunt пакет для ET с нуля (PH ещё не заходили): tagline≤60 + description + topics + first maker-коммент (launch-story) + галерея 1270×760 3-4 слайда (демо-гиф из et-how-to-long-v2 + карточки фото→текст/скрин→текст/Ctrl+V). Ассеты: assets/extract-text-icon-128.png, releases/video-samples/et-*.
- Жду от Дениса: (1) подтверждение запуска с его прогретого PH-аккаунта (не team@); (2) день — 07.08 чт или сдвиг на вт 12.08. PH старт 00:01 PST = 16:00 китайского времени Дениса.
- IE OBS-экранку НЕ просить сейчас (приберегаем).

## СЕССИЯ 2026-08-06 (продолжение) — Product Hunt ET: тексты готовы, ждём вычитку + галерею
### Что сделано
- Правило PH зафиксировано в offpage/PRODUCT-HUNT-RULES.md: запуск ВСЕГДА В ПЯТНИЦУ (микро-продукту нужна НИЗКАЯ конкуренция; хайп валит вт-ср — против него не тянем; пятница = шанс в лидеры дня). Аккаунт = прогретый ЛИЧНЫЙ Дениса. Я ошибся, предложив «вт-ср лучшие дни» (generic-совет) — Денис поправил, теперь в правилах.
- ЗАПУСК: пятница 07.08.2026, продукт Extract Text, личный прогретый аккаунт Дениса.
- Текстовый пакет ГОТОВ: offpage/product-hunt-extract-text.md — name, tagline (51/60 «Copy text from any image or screenshot in one click»), description, 3 topics (Productivity/Chrome Extensions/Design Tools), first maker-коммент (launch-story, on-device+no-paywall акценты), links+UTM.
### Следующий шаг
1. Денис вычитывает first maker-коммент (должен звучать его словами).
2. Рендер галереи 1270×760 — 3-4 слайда из ET-демо (кадр из et-how-to-long-v2 + карточки photo/screenshot/Ctrl+V→text). Thumbnail 240×240 из assets/extract-text-icon-128.png. НЕ собрана.
3. IE держим в резерве (флагманский залп позже). YouTube — дрип ET paste-short v2 параллельно.
